...
const personB: Person = {name: "Matheus", age: "21", address: {street: "lalala", number: 0, country: ""}}

describe("Comparing question mark and pipe operators", () => {
    it ("Should have the same behaviours, when person without country", () => {
        const countryPersonA_Question: string = personA.address?.country ?? "BR"
        const countryPersonA_Pipe: string = personA.address?.country || "BR"
    ...
    it ("Should have different behaviours, when person with empty country", () => {
        const countryPersonB_Question: string = personB.address?.country ?? "BR"
        const countryPersonB_Pipe: string = personB.address?.country || "BR"
